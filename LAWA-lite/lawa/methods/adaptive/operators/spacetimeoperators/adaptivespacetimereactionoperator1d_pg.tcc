...
AdaptiveSpaceTimeReactionOperator1D_PG<T, TrialBasis, TestBasis, TrialPrec, TestPrec, InitialCondition>::
    AdaptiveSpaceTimeReactionOperator1D_PG(const TrialBasis& _trialbasis, const TestBasis& _testbasis,
            TrialPrec& _trialprec, TestPrec& _testprec, T _reaction)
    : trialbasis(_trialbasis), testbasis(_testbasis), reaction(_reaction),
...
AdaptiveSpaceTimeReactionOperator1D_PG<T, TrialBasis, TestBasis, TrialPrec, TestPrec, InitialCondition>::
    AdaptiveSpaceTimeReactionOperator1D_PG(const TrialBasis& _trialbasis, const TestBasis& _testbasis,
            TrialPrec& _trialprec, TestPrec& _testprec, InitialCondition& _init_cond, T _reaction)
    : trialbasis(_trialbasis), testbasis(_testbasis), reaction(_reaction),
      compression_1d_t(_trialbasis.first), compression_1d_x(_trialbasis.second), compression(_trialbasis),
      P_trial_data(), P_test_data(), trialprec(_trialprec), testprec(_testprec), noprec(),
      op_identity_t(_trialbasis.first, _testbasis.first), op_identity_x(_trialbasis.second, _testbasis.second),
...
operator()(const Index2D &row_index, const Index2D &col_index)
{
    ...
    return prec * reaction * data_identity_t(row_index.index1,col_index.index1) * data_identity_x(row_index.index2,col_index.index2);
}
...
{
    std::cerr << "AdaptiveSpaceTimeReactionOperator1D_PG<T, TrialBasis, TestBasis, TrialPrec, TestPrec, InitialCondition>::"
