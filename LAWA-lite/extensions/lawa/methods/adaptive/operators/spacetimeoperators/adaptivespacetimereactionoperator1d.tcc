...
AdaptiveSpaceTimeReactionOperator1D<T, Basis2D, LeftPrec2D, RightPrec2D, InitialCondition>::
    AdaptiveSpaceTimeReactionOperator1D(const Basis2D& _basis, LeftPrec2D& _p_left, RightPrec2D& _p_right,
                                   T _reaction)
    : basis(_basis), reaction(_reaction),
...
AdaptiveSpaceTimeReactionOperator1D<T, Basis2D, LeftPrec2D, RightPrec2D, InitialCondition>::
    ...
    : basis(_basis), reaction(_reaction),
      compression_1d_t(_basis.first), compression_1d_x(_basis.second), compression(_basis),
      P_left_data(), P_right_data(), p_left(_p_left), p_right(_p_right), noprec(),
      op_identity_t(_basis.first), op_identity_x(_basis.second), op_noinitcond(), op_initcond(_init_cond),
      data_identity_t(op_identity_t,     noprec, compression_1d_t),
      data_identity_x(op_identity_x,     noprec, compression_1d_x)
{
}
...
{
    std::cerr << "AdaptiveSpaceTimeReactionOperator1D<T, Basis2D, LeftPrec2D, RightPrec2D, InitialCondition>::"
