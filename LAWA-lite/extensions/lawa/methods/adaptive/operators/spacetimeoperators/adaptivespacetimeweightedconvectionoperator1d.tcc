...
AdaptiveSpaceTimeWeightedConvectionOperator1D<T, Basis2D, LeftPrec2D, RightPrec2D, InitialCondition>::
    AdaptiveSpaceTimeWeightedConvectionOperator1D(const Basis2D& _basis, LeftPrec2D& _p_left, RightPrec2D& _p_right,
                                   Function<T> weightFct, T _convection)
    : basis(_basis), convection(_convection),
      ...
      op_identity_t(_basis.first), op_convection_x(_basis.second, weightFct), 
...
AdaptiveSpaceTimeWeightedConvectionOperator1D<T, Basis2D, LeftPrec2D, RightPrec2D, InitialCondition>::
    ...
    : basis(_basis), convection(_convection),
      compression_1d_t(_basis.first), compression_1d_x(_basis.second), compression(_basis),
      P_left_data(), P_right_data(), p_left(_p_left), p_right(_p_right), noprec(),
      op_identity_t(_basis.first), op_convection_x(_basis.second, weightFct), op_noinitcond(), op_initcond(_init_cond),
      data_identity_t(op_identity_t,     noprec, compression_1d_t),
      data_convection_x(op_convection_x, noprec, compression_1d_x)
{
}
